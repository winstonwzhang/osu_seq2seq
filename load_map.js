...
function parseGeneral(generalLines) {
    ...
    const intSet = ["AudioLeadIn", "PreviewTime", "Countdown", "Mode",
            ...
            "EpilepsyWarning", "UseSkinSprites"
        ],
        floatSet = ["StackLeniency"];
...
function getBezierPoly(points) {
    ...
    const _p = new Polynomial([0, 1]),
        _q = new Polynomial([1, -1]);
    var px = new Polynomial([0]),
        py = new Polynomial([0]);
    for (var i = 0; i < points.length; i++) {
        var ptimes = i,
            qtimes = points.length - i - 1,
            co = coeff[i];
...
function cutBezier(polys, target, prec) {
    ...
    for (var i = 0; i < table.length; i++) {
        if (tot == target) {
            ...
            return {
                t: s,
                c: [polys[0].eval(s), polys[1].eval(s)],
                l: tot
            };
    ...
    return {
        t: 1,
        c: [polys[0].eval(1), polys[1].eval(1)],
        l: target
    };
}
...
function analyzeSlider(note) {
    ...
    switch (sliderType) {
        ...
        case "C": // parse it as if it were linear; no one uses it anyways
            ...
            var firstLine = outCurves[0],
                lastLine = outCurves[outCurves.length - 1];
...
function makeClaps(param, hoa, dts, dte) {
    ...
    for (var i in hoa)
        if (!(hoa[i].time < dTimeStart || hoa[i].time > dTimeEnd))
            if (1 & hoa[i].type) switch (param) {
                case 1:
                    w = isWhiteLine(hoa[i].time), 2 == w || 4 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 2:
                    w = isWhiteLine(hoa[i].time), 1 == w || 3 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 3:
                    w = isWhiteLine(hoa[i].time, 3, .5), w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 4:
                    w = isWhiteLine(hoa[i].time), 2 == w || 3 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 5:
                    w = isWhiteLine(hoa[i].time), 2 == w || 4 == w ? hoa[i].hitsounds |= 2 : hoa[i].hitsounds &= 29;
                    break;
                case 6:
                    w = isWhiteLine(hoa[i].time), 2 == w ? (hoa[i].hitsounds |= 2, hoa[i].hitsounds &= 23) : 4 == w ? (hoa[i].hitsounds |= 8, hoa[i].hitsounds &= 29) : hoa[i].hitsounds &= 21;
                    break;
                case 7:
                    w = isWhiteLine(hoa[i].time), 4 == w ? (hoa[i].hitsounds |= 2, hoa[i].hitsounds &= 23) : 2 == w ? (hoa[i].hitsounds |= 8, hoa[i].hitsounds &= 29) : hoa[i].hitsounds &= 21;
                    break;
                case 8:
                    hoa[i].hitsounds = 0, hoa[i].extHitsounds = "0:0:0";
                    break;
                case 9:
                    8 & hoa[i].hitsounds || 2 & hoa[i].hitsounds ? hoa[i].hitsounds &= 21 : hoa[i].hitsounds |= 2;
                    break;
                case 10:
                    w = isWhiteLine(hoa[i].time), 1 == w ? hoa[i].hitsounds |= 4 : hoa[i].hitsounds &= 27;
                    break;
                case 11:
                    Math.random() > .5 ? hoa[i].hitsounds &= 21 : hoa[i].hitsounds |= 2;
                    break;
                case 12:
                    w = isWhiteLine(hoa[i].time), w2 = isWhiteLine(hoa[i].time, 3, .5), 2 == w || 4 == w || 3 == w2 ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 13:
                    w = isWhiteLine(hoa[i].time), 1 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 14:
                    w = isWhiteLine(hoa[i].time), 2 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 15:
                    w = isWhiteLine(hoa[i].time), 3 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23;
                    break;
                case 16:
                    w = isWhiteLine(hoa[i].time), 4 == w ? hoa[i].hitsounds |= 8 : hoa[i].hitsounds &= 23
            } else if (2 & hoa[i].type) {
                var ticks = hoa[i].sliderLength / getSliderLen(hoa[i].time),
                    tickLength = getTickLen(hoa[i].time);
                if (!hoa[i].sliderSingleHitsounds || hoa[i].sliderSingleHitsounds.length == 0) {
                    hoa[i].sliderSingleHitsounds = [];
                    for (var j = 0; j <= hoa[i].sliderReverses; j++) hoa[i].sliderSingleHitsounds.push(0)
                }
                if (!hoa[i].sliderExtHitsounds) {
                    hoa[i].sliderExtHitsounds = [];
                    for (j = 0; j <= hoa[i].sliderReverses; j++) hoa[i].sliderExtHitsounds.push("0:0");
                    hoa[i].extHitsounds = "0:0:0"
                }
                for (j = 0; j <= hoa[i].sliderReverses; j++) {
                    var c_tick = Math.round(hoa[i].time + j * ticks * tickLength);
                    switch (param) {
                        case 1:
                            w = isWhiteLine(c_tick), 2 == w || 4 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 2:
                            w = isWhiteLine(c_tick), 1 == w || 3 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 3:
                            w = isWhiteLine(c_tick, 3, .5), w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 4:
                            w = isWhiteLine(c_tick), 2 == w || 3 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 5:
                            w = isWhiteLine(c_tick), 2 == w || 4 == w ? hoa[i].sliderSingleHitsounds[j] |= 2 : hoa[i].sliderSingleHitsounds[j] &= 29;
                            break;
                        case 6:
                            w = isWhiteLine(c_tick), 2 == w ? (hoa[i].sliderSingleHitsounds[j] |= 2, hoa[i].sliderSingleHitsounds[j] &= 23) : 4 == w ? (hoa[i].sliderSingleHitsounds[j] |= 8, hoa[i].sliderSingleHitsounds[j] &= 29) : hoa[i].sliderSingleHitsounds[j] &= 21;
                            break;
                        case 7:
                            w = isWhiteLine(c_tick), 4 == w ? (hoa[i].sliderSingleHitsounds[j] |= 2, hoa[i].sliderSingleHitsounds[j] &= 23) : 2 == w ? (hoa[i].sliderSingleHitsounds[j] |= 8, hoa[i].sliderSingleHitsounds[j] &= 29) : hoa[i].sliderSingleHitsounds[j] &= 21;
                            break;
                        case 8:
                            hoa[i].sliderSingleHitsounds[j] = 0, hoa[i].sliderExtHitsounds[j] = "0:0";
                            break;
                        case 10:
                            w = isWhiteLine(c_tick), 1 == w ? hoa[i].sliderSingleHitsounds[j] |= 4 : hoa[i].sliderSingleHitsounds[j] &= 27;
                            break;
                        case 11:
                            Math.random() > .5 ? hoa[i].sliderSingleHitsounds[j] &= 21 : hoa[i].sliderSingleHitsounds[j] |= 2;
                            break;
                        case 12:
                            w = isWhiteLine(c_tick), w2 = isWhiteLine(c_tick, 3, .5), 2 == w || 4 == w || 3 == w2 ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 13:
                            w = isWhiteLine(c_tick), 1 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 14:
                            w = isWhiteLine(c_tick), 2 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 15:
                            w = isWhiteLine(c_tick), 3 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23;
                            break;
                        case 16:
                            w = isWhiteLine(c_tick), 4 == w ? hoa[i].sliderSingleHitsounds[j] |= 8 : hoa[i].sliderSingleHitsounds[j] &= 23
                    }
                }
            }
    return hoa;
...
function main() {
    ...
    var date0 = new Date();
    var mode = args[2].charAt(0),
        mode2 = args[2].charAt(1);
    ...
    if (mode == "j") {
        ...
        try {
            ...
            // this will overwrite it if it existed!!
            fs.writeFileSync(outputfile, buildOsuFile(json), {
                encoding: "utf8"
            });
            if (mode2 != "q") {
